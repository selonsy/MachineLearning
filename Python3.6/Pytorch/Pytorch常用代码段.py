...
'''指定程序运行在特定 GPU 卡上
'''
# CUDA_VISIBLE_DEVICES=0,1 python train.py    # 在命令行指定
...
'''
torch.cuda.empty_cache()               # 代码模式
# 命令行模式：先使用 ps 找到程序的 PID，再使用 kill 结束该进程
# ps aux | grep python
# kill -9 [pid]    
# 命令行模式2：直接重置没有被清空的 GPU
...
# 模型定义
# 建议有参数的层和汇合（pooling）层使用 torch.nn 模块定义，
# 激活函数直接使用 torch.nn.functional。
...
# 先将它们累积在 GPU 中，等一个 epoch 结束之后一起传输回 CPU 会比每个 mini-batch 都进行一次 GPU 到 CPU 的传输更快。
