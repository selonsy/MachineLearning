...
class SiamFPN(FPN):
    def __init__(self, block, feature_out=256, anchor=3):
        ...
        self.conv_reg1 = nn.Conv2d(
            256, feature_out*4*anchor, 3)      # 模板分支的回归子分支
        self.conv_reg2 = nn.Conv2d(
            256, feature_out, 3)               # 检测分支的回归子分支
        self.conv_cls1 = nn.Conv2d(
            256, feature_out*2*anchor, 3)    # 模板分支的分类子分支
        self.conv_cls2 = nn.Conv2d(
            256, feature_out, 3)             # 检测分支的分类子分支
    ...
    def forward(self, x):  # x:1,3,271,271
        ...
        for p_index in range(len(px)):
            ...
            delta = self.regress_adjust(
                F.conv2d(self.conv_reg2(px[p_index]), self.reg_kernel[p_index]))
            delta = bnn12(delta)
            # print(delta.size())
            ...
            # torch.Size([1, 12, 6, 6])
            socre = F.conv2d(self.conv_cls2(
                px[p_index]), self.cls_kernel[p_index])
            socre = bnn6(socre)
            # print(socre.size())
    ...
    def template(self, z):  # z:1,3,127,127
        ...
        for p in pz:
            ...
            # 6
            # 2
            reg_kernel_view = reg_kernel_raw.view(
            ...
            cls_kernel_view = cls_kernel_raw.view(
...
def test():
    net = FPN152().cuda()
    fms = net(Variable(torch.randn(1, 3, 127, 127)).cuda())
    for fm in fms:
        print(fm.size())
