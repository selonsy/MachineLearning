...
def worker(output_dir, video_dir):
    image_names = glob(os.path.join(video_dir, '*.JPEG')) # 通配符遍历
    # 以图片的名称排序，有错误，中间的分隔符不一致，但是先放着  D:\\workspace\\vot\\asimo\\HelloRicky-SiameseRPN\\Siamese-RPN\\ILSVRC\\Data/VID\\train/ILSVRC2015_VID_train_0000\\ILSVRC2015_train_00001000\\000000.JPEG
    # image_names = sorted(image_names,key=lambda x: int(x.split('/')[-1].split('.')[0])) # 以Key排序    
...
def processing(data_dir, output_dir, num_threads=mp.cpu_count()):
    ...
    #     # for ret in pool.imap_unordered(functools.partial(worker, output_dir), all_videos):
    #     #     meta_data.append(ret)
...
if __name__ == '__main__':
    # Fire(processing)
    data_dir = "D:\\workspace\\vot\\asimo\\HelloRicky-SiameseRPN\\Siamese-RPN\\ILSVRC"
    output_dir = "D:\\workspace\\vot\\asimo\\HelloRicky-SiameseRPN\\Siamese-RPN\\ILSVRC_Crops" 
    processing(data_dir,output_dir)
