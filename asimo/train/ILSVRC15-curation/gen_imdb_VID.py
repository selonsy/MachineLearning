    ...
    for i in range(len(all_dirs_level1)):
        ...
        for j in range(len(all_dirs_level2)):
            ...
            if vid_idx < train_video_num:
                ...
                if not all_dirs_level2[j] in videos_train:
                    videos_train[all_dirs_level2[j]] = []
            else:
                # if not videos_val.has_key(all_dirs_level2[j]):
                if not all_dirs_level2[j] in videos_val:
...
def generate_image_imdb4otb(vid_root_path, vid_curated_path):
    '''
    # save image crops to the vid_curated_path
    '''
    ...
    num_videos = 1  # 这里训练集只有一个，会导致验证集为空
    ...
    for i in range(len(img_list)):
        ...
        tmp_instance = dict()
        tmp_instance['instance_path'] = '{}.crop.x.jpg'.format(img_name)
        tmp_instance['bbox'] =bboxs[i]

        videos.append(tmp_instance)
    video["0"]=videos
    video_ids.append(video)
    if len(video_ids) > 0:
        if vid_idx < train_video_num:
            videos_train["bike"]=video_ids
        else:
            videos_val["bike"]=video_ids
    ...
    # save imdb information
    json.dump(imdb_video_train, open('imdb_video_train_otb.json', 'w'), indent=2)
    json.dump(imdb_video_val, open('imdb_video_val_otb.json', 'w'), indent=2)
...
if __name__ == "__main__":
    ...
    generate_image_imdb4otb(vid_root_path, vid_curated_path)
